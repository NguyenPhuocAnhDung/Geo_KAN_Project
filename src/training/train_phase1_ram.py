...
# ================= 2. BỘ ĐIỀU KHIỂN THỰC NGHIỆM CHÉO (CROSS-DATASET) =================
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.dirname(CURRENT_DIR)              

# LỰA CHỌN CHẾ ĐỘ CHẠY (Thay đổi giá trị này: "MERGED", "CICIoT2023", "CICDDoS2019")
DATASET_MODE = "MERGED" 

# Tự động cấu hình đường dẫn và tên lưu trữ dựa trên chế độ chọn
if DATASET_MODE == "MERGED":
    DATA_DIR = os.path.join(PROJECT_ROOT, "dataset", "processed", "Phase1_Train")
    EXPERIMENT_NAME = "v3_Attention_TKAN_FocalLoss_Merged"
elif DATASET_MODE == "CICIoT2023":
    DATA_DIR = os.path.join(PROJECT_ROOT, "dataset", "processed", "Phase1_Train", "CICIoT2023")
    EXPERIMENT_NAME = "v4_Single_CICIoT2023_Attention_TKAN"
elif DATASET_MODE == "CICDDoS2019":
    DATA_DIR = os.path.join(PROJECT_ROOT, "dataset", "processed", "Phase1_Train", "CICDDoS2019Full")
    EXPERIMENT_NAME = "v5_Single_CICDDoS2019_Attention_TKAN"
else:
    raise ValueError("DATASET_MODE không hợp lệ!")

PLOT_DIR = os.path.join(PROJECT_ROOT, "reports", "plots", EXPERIMENT_NAME)
MODEL_SAVE_DIR = os.path.join(PROJECT_ROOT, "models", "checkpoints", EXPERIMENT_NAME)
...
device = torch.device("cpu") # Chạy trên RAM/CPU
...
NUM_WORKERS = 32 # Tận dụng 64 core CPU

# ================= LOGGING =================
LOG_FILE = os.path.join(LOG_DIR, f"training.log")
logging.basicConfig(
    level=logging.INFO, 
...
# ================= DATASET (Đã loại bỏ lỗi 80 vs 114 cột) =================
class TimeSeriesDataset(Dataset):
    def __init__(self, df, seq_length=10, label_mapping=None, feature_cols=None, scaler=None):
        X_df = df.drop(columns=['Label'], errors='ignore')
        if feature_cols is not None:
            # Tự động chèn 0 nếu file thiếu cột
            X_df = X_df.reindex(columns=feature_cols, fill_value=0.0)
        if scaler is not None:
            self.features = scaler.transform(X_df.values).astype(np.float32)
        else:
...
class EarlyStopping:
    def __init__(self, patience=7):
        self.patience = patience; self.counter = 0; self.best_loss = None; self.early_stop = False
    def __call__(self, val_loss):
        if self.best_loss is None: self.best_loss = val_loss
...
def train():
    all_files = glob.glob(os.path.join(DATA_DIR, "**/*.parquet"), recursive=True)
    if not all_files: return logging.error(f"❌ Không thấy dữ liệu tại {DATA_DIR}!")

    logging.info(f"🚀 BẮT ĐẦU CHẠY THỰC NGHIỆM: {EXPERIMENT_NAME}")
    logging.info(f"📂 Đang nạp dữ liệu từ: {DATA_DIR}")
    
    global_scaler = joblib.load(SCALER_PATH)
    GLOBAL_FEATURE_COLS = global_scaler.feature_names_in_.tolist() # Lấy 114 cột chuẩn
    
    # Quét nhãn
    temp_labels = set()
    for f in tqdm(all_files, desc="Quét Labels"):
        temp_labels.update(pq.read_table(f, columns=['Label']).to_pandas()['Label'].unique())
    GLOBAL_LABEL_MAP = {lbl: idx for idx, lbl in enumerate(sorted(list(temp_labels)))}
    class_names = [k for k, v in sorted(GLOBAL_LABEL_MAP.items(), key=lambda item: item[1])]
    
    # Nạp RAM
    train_datasets_list, val_datasets_list = [], []
    global_class_counts = {i: 0 for i in range(len(GLOBAL_LABEL_MAP))}
    for file_path in tqdm(all_files, desc="Nạp RAM & Chia"):
        full_df = pq.read_table(file_path).to_pandas()
        t_list, v_list = [], []
        for label, group in full_df.groupby('Label'):
            mapped_label = GLOBAL_LABEL_MAP[label]
            ...
            t_list.append(group.iloc[:split_idx])
            if split_idx < len(group): v_list.append(group.iloc[split_idx:])
        if t_list: train_datasets_list.append(TimeSeriesDataset(pd.concat(t_list), SEQ_LENGTH, GLOBAL_LABEL_MAP, GLOBAL_FEATURE_COLS, global_scaler))
        if v_list: val_datasets_list.append(TimeSeriesDataset(pd.concat(v_list), SEQ_LENGTH, GLOBAL_LABEL_MAP, GLOBAL_FEATURE_COLS, global_scaler))

    # Tính trọng số Smoothed
    class_weights = [1.0 / np.sqrt(global_class_counts[i] + 1e-5) for i in range(len(GLOBAL_LABEL_MAP))]
    class_weights_tensor = torch.FloatTensor(class_weights).to(device)

    train_loader = DataLoader(ConcatDataset(train_datasets_list), batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS)
    val_loader = DataLoader(ConcatDataset(val_datasets_list), batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS)

    # Khởi tạo mô hình Attention_TKAN
    ...
    for epoch in range(1, 101):
        ...
        with torch.no_grad():
            for x, y in tqdm(val_loader, desc=f"🔍 [VALID] Ep {epoch}", dynamic_ncols=True):
                ...
                v_preds.extend(out.argmax(1).cpu().numpy()); v_labels.extend(y.cpu().numpy())
        
        avg_v_loss = v_loss / len(val_loader)
        
        # Báo cáo đánh giá và Lưu file
        report = classification_report(v_labels, v_preds, target_names=class_names, digits=4, zero_division=0)
        logging.info(f"\n📊 BÁO CÁO KẾT QUẢ EPOCH {epoch}:\n{report}")
        logging.info(f"👉 Validation Loss: {avg_v_loss:.4f}")
        
        plot_confusion_matrix(v_labels, v_preds, class_names, f"Ep {epoch}", f"cm_epoch_{epoch}")
        
        # LƯU MÔ HÌNH ĐÃ TRAIN VÀO THƯ MỤC CỦA TỪNG BỘ DỮ LIỆU
        torch.save(model.state_dict(), os.path.join(MODEL_SAVE_DIR, f"best_model_ep{epoch}.pth"))
        
        scheduler.step(avg_v_loss)
        early_stopping(avg_v_loss)
        if early_stopping.early_stop: 
            logging.info(f"🛑 Kích hoạt Early Stopping! Mô hình hội tụ tại Epoch {epoch}.")
            break
