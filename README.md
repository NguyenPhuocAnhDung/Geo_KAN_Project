# Geo KAN Project
[![Ask DeepWiki](https://devin.ai/assets/askdeepwiki.png)](https://deepwiki.com/NguyenPhuocAnhDung/Geo_KAN_Project)
...
1.  **LSTM Layer (`nn.LSTM`):** The first layer processes the input time-series data (sequences of network flow statistics) to capture temporal dependencies and patterns over time.
2.  **MLP Block (`nn.Sequential`):** The output from the LSTM is passed through a Multi-Layer Perceptron consisting of `Linear` layers, `LayerNorm`, and `GELU` activation functions. This block further processes and refines the learned temporal features.
3.  **Chebyshev KAN Layer (`ChebyshevKANLayer`):** The final classification is performed by a custom Kolmogorov-Arnold Network layer. Instead of a simple linear transformation, this layer uses a basis of Chebyshev polynomials to learn complex activation functions, potentially leading to better modeling of intricate relationships in the data.
...
1.  Place your raw network traffic datasets (in `.csv` or `.parquet` format) into the `dataset/raw/` directory. The preprocessing script expects a structure like `dataset/raw/<DatasetName>/<files...>`.
2.  Run the data preprocessing script. This will process all datasets defined in `DATASET_PHASES`, standardize them, and save the cleaned output to `dataset/processed/`.
